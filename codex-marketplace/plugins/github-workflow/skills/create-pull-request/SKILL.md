...
description: Create a GitHub pull request from the current working tree by assessing changes, creating a branch from main when needed, staging, committing, pushing, and opening the PR through the bundled GitHub wrapper in this plugin. Use when the user asks to create a PR, open a PR, push and create a PR, make a PR, ship the current changes, or otherwise wants the current work turned into a pull request.
---
...
Always use the bundled wrapper at `<plugin-root>/scripts/gh` with `CODEX_GH_PROFILE=doppelganger` so the PR is created as `tamakiii-doppelganger[bot]`. Resolve `<plugin-root>` from the installed plugin path before running commands. Always pass `--repo` explicitly.

Before creating a new PR, check whether one already exists for the current branch:

```bash
CODEX_GH_PROFILE=doppelganger <plugin-root>/scripts/gh pr list --repo <owner/repo> --head <branch-name> --json number,url
```
...
```bash
CODEX_GH_PROFILE=doppelganger <plugin-root>/scripts/gh pr create --repo <owner/repo> --title "<title>" --body "$(cat <<'EOF'
## Summary
- <what changed>
- <why it changed>
