...
description: "Automate the full PR creation workflow: detect changes, create a branch from main, stage, commit, push, and open a PR via ~/.local/bin/gh as tamakiii-doppelganger[bot]. Trigger whenever the user says 'create a PR', 'create new PR', '/create-pull-request', '/create-pr', 'push and create a PR', 'open a PR', 'make a PR', 'PR this', or any variation that implies they want their current changes turned into a pull request. Also trigger when the user finishes a task and says something like 'ship it' or 'send it' in the context of creating a PR."
---
...
Use `~/.local/bin/gh` (not plain `gh`) to create the PR as `tamakiii-doppelganger[bot]`. Always specify `--repo` explicitly using the value derived in Step 1.
...
- Always use `~/.local/bin/gh` for PR creation — this posts as `tamakiii-doppelganger[bot]`. Never use bare `gh`.
